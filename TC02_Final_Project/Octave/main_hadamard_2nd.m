        ...
        for type = 1:length(mod_schemes) % Loop over modulation schemes
          ...
          for snr_idx = 1:length(SNR_values)
              ...
              for i = 1:num
                  ...
                  if (type == 1)
                    [x, x_hat] = otfs_hadamard_2nd(N, M, spd, fc, delta_f, SNR_db, mod_size, delays_arr, pdp_arr, "127.0.0.1", 5016);
                  elseif (type == 2)
              ...
              filename = 'ber_values_2nd.txt';
              file = fopen(filename, 'a'); % Open file in append mode
              if file == -1
                  error('Could not open file for writing.');
              end
              dateAndTime = datestr(now(), 'yyyy_mmmm_dd_HH-MM-SS');
              fprintf(file, 'Date: %s: ,Scenario Index: %d, Type: %d, Modulation: %d-QAM, SNR: %f, BER: %e\n', dateAndTime, C, type, mod_size, SNR_db, BER_values(type, snr_idx));
              fclose(file);
          end
        end
