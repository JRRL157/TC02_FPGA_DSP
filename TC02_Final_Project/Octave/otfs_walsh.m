function [x, x_hat2] = otfs_walsh(N, M, spd, fc, delta_f, SNR_db, mod_size, delays_arr, pdp_arr)
  Wn = fwht(eye(N), N, 'sequency');
  Wm = fwht(eye(M), M, 'sequency');
  Wn=Wn/norm(Wn);
  Wm=Wm/norm(Wm);
