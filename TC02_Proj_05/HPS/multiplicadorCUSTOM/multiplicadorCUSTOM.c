...
//Dual Port RAM
#define DP_RAM_BASE_ADDR 0x40010
...
int main()
{
	...
	while(1)
	{
		...
		peripheral_write8(perif, DATA_IN_OFFSET, numbers);
		...
		peripheral_write8(perif, CONTROL_OFFSET, 0x1);
		printf("Wrote %x in address %x\n", 0x1, DP_RAM_BASE_ADDR + CONTROL_OFFSET);

		do {
			usleep(100000);
			status = peripheral_read8(perif, STATUS_OFFSET);
			printf("Status = %x\n", status);
		} while(status == 0);

		data_out = peripheral_read8(perif, DATA_OUT_OFFSET);
		printf("Data out = %x, %u\n", data_out, data_out);
